...
def profile():
    ...
    for alert in my_alerts:
        alert.attr_filters = decode_alert_payload(alert.attr_filters)
    my_notifications = OutbidNotification.query.filter_by(user_id=user.user_id).order_by(
        OutbidNotification.created_at.desc()).limit(20).all()
    ...
    return render_template('profile.html', user=user, my_items=my_items,
                           my_bids=my_bids, my_alerts=my_alerts,
...
def toggle_anonymous():
    ...
    flash(f'Anonymous bidding {state}.', 'info')
...
def mark_notifications_read():
    OutbidNotification.query.filter_by(user_id=session['user_id'], is_read=False).update({'is_read': True})
    db.session.commit()
    return redirect(url_for('profile'))
...
def questions():
    ...
    query = SupportQuestion.query.filter_by(status='answered')
    if q:
        query = query.filter(
            SupportQuestion.subject.ilike(f'%{q}%') |
            SupportQuestion.question.ilike(f'%{q}%') |
            SupportQuestion.answer.ilike(f'%{q}%')
        )
    questions = query.order_by(SupportQuestion.answered_at.desc()).all()
    return render_template('questions.html', questions=questions, q=q)
